...
inline void shared_ptr_storage<T>::removeReference() {
	...
	if (myCounter == 0) {
		T* ptr = myPointer;
		myPointer = 0;
        //LOGI("mobi******removeReference myStorage : %4x",ptr);
		delete ptr;
	}
}
template<class T>
...
inline void shared_ptr<T>::attachStorage(shared_ptr_storage<T> *storage) {
	myStorage = storage;
	if (myStorage != 0) {
        //LOGI("mobi******attachStorage myStorage : %4x counter ： %d ,this : %s",myStorage,myStorage->counter(),typeid(T).name());
...
inline void shared_ptr<T>::detachStorage() {
	if (myStorage != 0) {
		...
		if (myStorage->counter() == 1) {
            //LOGI("mobi******detachStorage myStorage : %4x counter ： %d this : %s",myStorage,myStorage->counter() , typeid(T).name());
