...
bool ZLStringUtil::charIsNumber(const char c){
    if(c >= '0' && c <= '9')
        return true;
    else
        return false;
...
void ZLStringUtil::eraseLineBreaks(std::string &src)
{
	std::size_t pos = 0;
	pos=src.find('\n', pos);
	while ((pos != std::string::npos))
	{
		src.replace(pos,pos+1," ");
		pos=src.find('\n',pos);
	}
}

bool ZLStringUtil::IsUTF8(const void* pBuffer, long size)
{
	bool IsUTF8 = true;
